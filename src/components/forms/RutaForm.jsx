...
const RutaForm = ({ onSubmit, localidades, initialData = null, isEditing = false }) => {
  ...
  // Estado para el diálogo del selector de hora
  ...
  const agregarSalida = () => {
    ...
    setSalidas(prev => [...prev, nuevaSalidaObj]);
  ...
  const handleTimeSelected = (newTime) => {
    handleChangeSalida('hora', newTime);
    setTimePickerOpen(false);
  };
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12}>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
            ...
            <Button
                variant="contained"
                color={(!formData.codigo || !formData.origen || !formData.destino) ? "disabled" : "primary"}
                startIcon={<SaveIcon />}
        ...
        <Grid item xs={12} md={6}>
          <Typography variant="h6" gutterBottom>Información de la Ruta</Typography>
        ...
        <Grid item xs={12} md={6}>
          <Typography variant="h6" gutterBottom>Horarios de Salida</Typography>
          ...
          <Box sx={{ mb: 3, p: 2, border: '1px solid #e0e0e0', borderRadius: 1 }}>
            <Grid container spacing={2} alignItems="center">
              ...
              <Grid item xs={12} sm={5}>
                    ...
                    <LocalizationProvider dateAdapter={AdapterMoment}>
                      <TimePicker
                        ...
                        onChange={(newValue) => handleChangeSalida('hora', newValue)}
                        ampm={true}
                        viewRenderers={{
              ...
              <Grid item xs={12} sm={2}>
                <Button
                  variant="contained"
                  color={(!nuevaSalida.dia || !nuevaSalida.hora)? "disabled" : "primary"}
          ...
          <Card variant="outlined">
            <CardContent sx={{ p: 0, "&:last-child": { pb: 0 } }}>
              <List dense sx={{ width: '100%', maxHeight: '300px', overflow: 'auto' }}>
                {salidas.length === 0 ? (
                  <ListItem>
                  ...
                  salidas.map((salida) => (
                    <ListItem key={salida.id} divider>
                      ...
                      />
                      <ListItemSecondaryAction>
                        <IconButton edge="end" aria-label="delete" onClick={() => eliminarSalida(salida.id)}>
                          <DeleteIcon />
                        </IconButton>
                      </ListItemSecondaryAction>
                    </ListItem>
          ...
          <FormHelperText>*Las salidas serán guardadas junto con la información de la ruta</FormHelperText>
        </Grid>
      </Grid>
    </Paper>
  );
